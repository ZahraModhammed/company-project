 ...
 document.addEventListener("scroll", (e) => {
    ...
    if(document.body.classList.contains("landing-page")){
        ...
        for(let n = 0; n < navlink.length; n++){
            navlink[n].addEventListener("click", function (e){
              let terget = "#" + navlink[n].getAttribute("href");
              e.preventDefault();
              window.scrollTo({
                top:document.querySelector(terget).offsetTop - 70,
                 behavior: "smooth"
              })
            });
        }
    ...
    if (document.body.classList.contains("landing-page")) {
        ...
        menuButton.addEventListener("click", function () { //on click on menu in phone size ==>
            nav.classList.toggle("navbar--show-fa-bars"); //add style to nav to change background and color if no scroll in action
            menu.classList.toggle("show-list"); //show menu list with this class
        });
